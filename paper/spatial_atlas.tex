...
\begin{enumerate}[leftmargin=*,itemsep=2pt]
  ...
  \item \textbf{Leak Audit Registry}: A prompt-based exploit framework that detects train/test data leakage at codegen time and injects targeted hints so the strong model can adapt the exploit to the actual data layout.
\end{enumerate}
...
\paragraph{Three-Tier Frontier Model Routing.} We define three model tiers: \emph{fast}, \emph{standard}, and \emph{strong}, each mapped to a distinct model drawn from two frontier providers as shown in Table~\ref{tab:model_tiers}. The routing decision is based on task complexity, estimated by the entropy-guided reasoning engine (Section~\ref{sec:entropy}).
...
Strong   & Claude Opus 4.6         & \$15.00 / \$75.00  & $\sim$6s \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
Tabular     & Classif./Regression & LightGBM/XGBoost, feature engineering, cross-validation \\
NLP         & Text Classif./NER   & Transformer fine-tuning, TF-IDF fallback \\
Vision      & Image Classif.      & Pre-trained CNN, transfer learning, augmentation \\
TimeSeries  & Forecasting         & Prophet, ARIMA, lag features, rolling statistics \\
General     & Mixed/Unknown       & Ensemble of lightweight models with voting \\
...
\subsection{Score-Driven Refinement Loop}
...
\paragraph{Leak Audit Effectiveness.}
...
Anthropic.
\newblock Claude model family: Claude Opus 4.6, Sonnet 4.6.
...
\newblock Available at \url{https://github.com/Significant-Gravitas/AutoGPT}, 2023.
